...
function trackedMembers(guild, roleId) {
    if (!roleId) return [];
    return [...guild.members.cache.values()].filter(member => member.roles.cache.has(roleId));
}
...
function buildLiveStatus(guild) {
    ...
    const lines = active.length ? active.map(member => `🟢 <@${member.id}>`) : ["⚫ No staff currently marked online."];
    return [
        "╔══════════════════════════════╗",
        "      📊 **STAFF ATTENDANCE**",
        "╚══════════════════════════════╝",
...
async function updateLiveStatus(guild) {
    ...
    let message = null;
    if (config.statusMessageId) message = await channel.messages.fetch(config.statusMessageId).catch(() => null);
    try {
        if (message) {
        ...
        console.error(`[ATTENDANCE] Live status update failed: ${error.message} (code ${error.code || "unknown"})`);
        return false;
    }
}
function attendancePanelComponents() {
    ...
    return [new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId("attendance:mark-online").setLabel("Mark Online").setEmoji("🟢").setStyle(ButtonStyle.Success)
    )];
}
async function sendAttendancePanel(guild) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setTimestamp();
    try {
        await channel.send({ embeds: [embed], components: attendancePanelComponents() });
        return true;
    } catch (error) {
...
async function sendAttendanceMessage(guild, member, type, sessionSeconds = 0, reason = "", sessionStart = null, sessionEnd = null) {
    ...
    try {
        ...
        console.error(`[ATTENDANCE] Send failed: ${error.message} (code ${error.code || "unknown"})`);
        return false;
    }
}
async function sendDailySummary(guild) {
    ...
    for (const [userId, record] of Object.entries(data.members || {})) {
        ...
        if (seconds > 0) entries.push({ member, seconds, online: isOnline(member) });
    }
    entries.sort((a, b) => b.seconds - a.seconds);
    const online = entries.filter(x => x.online).length;
    let content = `📊 **DAILY STAFF ATTENDANCE**\n📅 ${ts(Date.now(), "D")}\n🟢 Currently online: **${online}**\n👥 Staff with activity today: **${entries.length}**\n\n`;
    content += entries.length ? entries.map((entry, i) => `${i + 1}. ${entry.online ? "🟢" : "⚫"} <@${entry.member.id}> — **${formatDuration(entry.seconds)}**`).join("\n") : "No attendance recorded today.";
    try {
        await channel.send({ content });
        return true;
    } catch (error) {
...
async function handlePresence(member, state, announceChanges = true, reason = "") {
    ...
    if (state === "online") {
        if (!record.activeSince) {
            ...
            saveAttendance(data);
            if (announceChanges) await sendAttendanceMessage(member.guild, member, "online", 0, reason, start, null);
            else await updateLiveStatus(member.guild);
        }
        return;
    }
    if (state === "offline") {
        ...
        saveAttendance(data);
        if (announceChanges) await sendAttendanceMessage(member.guild, member, "offline", seconds, reason, start, end);
        else await updateLiveStatus(member.guild);
    }
}
function getStats(userId) {
    ...
    return {
        ...
        currentSession: record.activeSince ? Math.max(0, (Date.now() - record.activeSince) / 1000) : 0
    };
}
